...
csvwfile = open('C:\\Downloads\\Clustered_mahattan_09.tsv', 'w',newline='')
my_writer = csv.writer(csvwfile, delimiter='\t')

with open('C:\\Downloads\\For_clustering_09.tsv',newline='') as csvfile:
    my_reader = csv.reader(csvfile, delimiter='\t')
    next(my_reader)
    for row in my_reader:
        clust_dict[row[2]].append(row[1])

with open('C:\\Downloads\\Manhattan_09.tsv',newline='') as csvfile:
    my_reader = csv.reader(csvfile, delimiter='\t')
    for row in my_reader:
        row_to_write = row
        clust_list = []
